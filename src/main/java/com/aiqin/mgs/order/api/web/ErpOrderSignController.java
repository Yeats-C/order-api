...
public class ErpOrderSignController {
    ...
    @Resource
    private ErpOrderSignService erpOrderSignService;
    ...
    public HttpResponse getOrderSignDetail(@RequestBody OrderStoreOrderInfo orderStoreOrderInfo) {
        ...
        try {
            OrderStoreOrderInfo orderSignDetail = erpOrderSignService.getOrderSignDetail(orderStoreOrderInfo);
    ...
    public HttpResponse orderSign(@RequestBody OrderStoreOrderInfo orderStoreOrderInfo) {
        HttpResponse response = HttpResponse.success();
        try {
            erpOrderSignService.orderSign(orderStoreOrderInfo);
        } catch (BusinessException e) {
            logger.error("异常信息：{}", e);
            response = HttpResponse.failure(MessageId.create(Project.ORDER_API, 99, e.getMessage()));
        } catch (Exception e) {
