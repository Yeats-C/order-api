...
public class ErpOrderCancelServiceImpl implements ErpOrderCancelService {
    ...
    public void cancelOrderWithoutStock(ErpOrderInfo erpOrderInfo) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void cancelOrderRejectSign(ErpOrderInfo erpOrderInfo) {
        if (erpOrderInfo == null) {
            throw new BusinessException("空参数");
        }
        if (StringUtils.isEmpty(erpOrderInfo.getOrderCode())) {
        ...
        if (orderTypeEnum.getErpProductTypeEnum() == ErpProductTypeEnum.DISTRIBUTION) {
            //配送订单
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void applyCancelOrder(ErpOrderInfo erpOrderInfo) {
    ...
    @Override
    public void orderCancelResultCallback(ErpOrderCancelResultRequest erpOrderCancelResultRequest) {

        AuthToken auth = AuthUtil.getCurrentAuth();

        if (erpOrderCancelResultRequest == null || StringUtils.isEmpty(erpOrderCancelResultRequest.getOrderCode())) {
            throw new BusinessException("缺失订单编号");
        }
        if (erpOrderCancelResultRequest.getStatus() == null) {
            throw new BusinessException("缺失状态");
        }
        if (YesOrNoEnum.exist(erpOrderCancelResultRequest.getStatus())) {
            throw new BusinessException("不合法的参数");
        }
        ErpOrderInfo order = erpOrderQueryService.getOrderByOrderCode(erpOrderCancelResultRequest.getOrderCode());
        if (order == null) {
        ...
        if (YesOrNoEnum.YES.getCode().equals(erpOrderCancelResultRequest.getStatus())) {
            //可以取消
